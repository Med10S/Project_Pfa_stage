...
# ============================================================================
# ETERNALBLUE SOAR INTELLIGENCE EXTRACTOR v3.0
# Advanced Multi-Phase Detection and PCAP Analysis
# ============================================================================
# Author: SOC Automation Team
# Date: 2025-08-03
# Purpose: Complete EternalBlue attack lifecycle detection and analysis
# 
# Features:
...
# ============================================================================
source "anti_duplication_patch.sh"
...
# Create directories
mkdir -p "$FILTERED_PCAP_DIR" "$TEMP_DIR" "$CORRELATION_DIR"

# Attack phase definitions (based on Suricata rule SIDs)
declare -A ATTACK_PHASES=(
    ...
    ["9000009"]="POST_EXPLOIT_DOUBLEPULSAR"
    ["9000010"]="POST_EXPLOIT_NAMEDPIPE"
    ...
    ["9000030"]="MEDICAL_TARGET"
    # Legacy rule support
    ["10001254"]="PHASE_1_LEGACY_TRANS2"
    ["1000012"]="PHASE_1_LEGACY_SMB3"
)

# Priority levels for different phases
...
# PCAP capture window based on phase (seconds)
...
log_message() {
    echo "$(date '+%Y-%m-%d %H:%M:%S') - [ETERNALBLUE-SOAR] $1" | tee -a "$LOG_FILE" >&2
}

# ============================================================================
# ATTACK CORRELATION MANAGEMENT
# ============================================================================

# Store attack event for correlation analysis
store_attack_event() {
    local src_ip="$1"
    local dest_ip="$2"
    local timestamp="$3"
    ...
    # Create or update correlation file
...
EOF
    ...
    # Add to correlation file (append to array)
...
# Analyze attack progression for a session
analyze_attack_progression() {
    ...
    log_message "   🔍 Phases detected: $(echo "$phases_detected" | tr '\n' ', ')"
    log_message "   📊 Total events: $total_events"
    log_message "   ⏱️ Time span: ${time_span}s"
    log_message "   🎯 Current phase: $current_phase"
...
# Test and repair PCAP files (from original script, enhanced)
test_and_fix_pcap() {
    local pcap_file="$1"
    local base_name=$(basename "$pcap_file")

    log_message "🔍 Testing PCAP integrity: $base_name"

    # Quick corruption test
    local test_output=$(timeout 10s tcpdump -r "$pcap_file" -c 1 2>&1)
    local test_result=$?
    ...
    if echo "$test_output" | grep -q "truncated dump file\|corrupt"; then
        ...
        log_message "🔧 Attempting PCAP repair..."
        
        if timeout 30s tcpdump -r "$pcap_file" -w "$fixed_file" 2>/dev/null; then
            if [ -f "$fixed_file" ] && [ -s "$fixed_file" ]; then
                local packet_test=$(timeout 10s tcpdump -r "$fixed_file" -c 1 2>/dev/null | wc -l || echo "0")
                if [ "$packet_test" -gt 0 ]; then
        ...
        log_message "✅ PCAP file healthy: $base_name"
        echo "$pcap_file"
        return 0
    else
        log_message "❌ PCAP file unreadable: $base_name"
        return 1
    fi
}

# Enhanced traffic extraction with phase-specific logic
extract_eternalblue_traffic() {
    ...
    log_message "=== ENHANCED ETERNALBLUE EXTRACTION ==="
    log_message "Attack Phase: $phase"
    log_message "Source: $src_ip → Target: $dest_ip"
    log_message "Priority: ${PRIORITY_LEVELS[$phase]}"
    
    # Get phase-specific capture window
    local capture_window="${CAPTURE_WINDOWS[$phase]:-30}"
    log_message "📏 Capture window: ${capture_window}s"
    ...
    # Force rotation before extraction for recent alerts
    force_rotation_before_extraction
    
    # Find candidate PCAP files with extended time range
    local start_time_epoch=$(($(date -d "$timestamp" +%s) - capture_window))
    local end_time_epoch=$(($(date -d "$timestamp" +%s) + capture_window))
    
    log_message "🔍 Searching PCAPs in time range: $(date -d "@$start_time_epoch") to $(date -d "@$end_time_epoch")"
    
    # Find files modified within our time window - NOUVEAU FORMAT SURICATA
    local candidate_files=$(find "$PCAP_DIR" -name "time_block.pcap.*" -type f -newermt "@$start_time_epoch" ! -newermt "@$end_time_epoch" -printf '%T@ %p\n' 2>/dev/null | sort -nr | cut -d' ' -f2-)
    
    if [ -z "$candidate_files" ]; then
        log_message "⚠️ No time_block PCAPs in time window, trying broader search..."
        candidate_files=$(find "$PCAP_DIR" -name "time_block.pcap.*" -type f -mmin -$((capture_window/60 + 5)) -printf '%T@ %p\n' 2>/dev/null | sort -nr | cut -d' ' -f2-)
    fi
    
    # Fallback: search any .pcap files if time_block format not found
    if [ -z "$candidate_files" ]; then
        log_message "⚠️ No time_block files found, searching any PCAP files..."
        candidate_files=$(find "$PCAP_DIR" -name "*.pcap*" -type f -newermt "@$start_time_epoch" ! -newermt "@$end_time_epoch" -printf '%T@ %p\n' 2>/dev/null | sort -nr | cut -d' ' -f2-)
    fi
    ...
    log_message "📂 Found $files_count candidate PCAP files"
    
    # Test and repair PCAP files
    while IFS= read -r pcap_file; do
        if [ -z "$working_pcap" ] && [ -f "$pcap_file" ]; then
            log_message "🧪 Testing: $(basename "$pcap_file")"
            
            local fixed_file=$(test_and_fix_pcap "$pcap_file")
            local fix_result=$?
            
            if [ $fix_result -eq 0 ] && [ -n "$fixed_file" ] && [ -f "$fixed_file" ]; then
                # Verify packets exist
                local packet_count=$(timeout 15s tcpdump -r "$fixed_file" 2>/dev/null | wc -l || echo "0")
                if [ "$packet_count" -gt 0 ]; then
                    log_message "✅ Working PCAP: $(basename "$fixed_file") ($packet_count packets)"
    ...
    if [ -z "$working_pcap" ]; then
        log_message "❌ No usable PCAP files found"
        send_alert_extraction_failed "$src_ip" "$dest_ip" "$timestamp" "$signature" "$phase" "$attack_status"
        return 1
    fi
    
    # Create output filename with phase information
    ...
    log_message "📤 Extracting to: $output_filename"
    
    # Enhanced extraction with phase-specific filters
    local success=false
    local used_filter=""
    ...
    if [[ "$phase" =~ ^PHASE_[23]|POST_EXPLOIT|CORRELATION ]]; then
        # For advanced phases, capture broader traffic including responses
        local filter_bidirectional="((src host $src_ip and dst host $dest_ip) or (src host $dest_ip and dst host $src_ip)) and (port 445 or port 139 or port 135)"
        log_message "🔍 Advanced phase filter: $filter_bidirectional"
        
        if timeout 60s tcpdump -r "$working_pcap" -w "$output_path" "$filter_bidirectional" 2>/dev/null; then
            if [ -f "$output_path" ] && [ -s "$output_path" ]; then
                local packet_count=$(timeout 20s tcpdump -r "$output_path" 2>/dev/null | wc -l || echo "0")
                if [ "$packet_count" -gt 0 ]; then
                    success=true
                    used_filter="$filter_bidirectional"
                    extraction_method="phase_specific_bidirectional"
                    log_message "✅ Advanced extraction successful: $packet_count packets"
                fi
            fi
        fi
    else
        # For initial phases, use standard extraction
        local filter_standard="((src host $src_ip and dst host $dest_ip) or (src host $dest_ip and dst host $src_ip)) and port 445"
        log_message "🔍 Standard filter: $filter_standard"
        
        if timeout 60s tcpdump -r "$working_pcap" -w "$output_path" "$filter_standard" 2>/dev/null; then
            if [ -f "$output_path" ] && [ -s "$output_path" ]; then
                local packet_count=$(timeout 20s tcpdump -r "$output_path" 2>/dev/null | wc -l || echo "0")
                if [ "$packet_count" -gt 0 ]; then
                    success=true
                    used_filter="$filter_standard"
                    extraction_method="standard_smb"
                    log_message "✅ Standard extraction successful: $packet_count packets"
                fi
            fi
        fi
    fi
    
    # Fallback: broader capture if specific filters fail
    if [ "$success" = false ]; then
        rm -f "$output_path" 2>/dev/null
        local filter_fallback="host $src_ip or host $dest_ip"
        log_message "🔍 Fallback filter: $filter_fallback"
        
        if timeout 60s tcpdump -r "$working_pcap" -w "$output_path" "$filter_fallback" 2>/dev/null; then
            if [ -f "$output_path" ] && [ -s "$output_path" ]; then
                local packet_count=$(timeout 20s tcpdump -r "$output_path" 2>/dev/null | wc -l || echo "0")
                if [ "$packet_count" -gt 0 ]; then
                    success=true
                    used_filter="$filter_fallback"
                    extraction_method="fallback_broad"
    ...
    if [ "$is_temp_file" = true ]; then
        rm -f "$working_pcap"
        log_message "🧹 Temporary PCAP cleaned up"
    fi
    
    # Send results
    if [ "$success" = true ]; then
        local output_size=$(stat -c%s "$output_path" 2>/dev/null || echo "0")
        log_message "🎉 ETERNALBLUE EXTRACTION SUCCESSFUL: $output_size bytes"
        send_alert_with_pcap "$src_ip" "$dest_ip" "$timestamp" "$signature" "$output_path" "$phase" "$attack_status" "$extraction_method" "$used_filter"
    else
...
send_alert_with_pcap() {
    ...
    # Enhanced analysis for advanced phases
    local advanced_analysis=""
    if [[ "$phase" =~ ^PHASE_[23]|POST_EXPLOIT|CORRELATION ]]; then
        # Perform additional packet analysis
...
force_rotation_before_extraction() {
    ...
    local recent_files=$(find "$PCAP_DIR" -name "*.pcap" -newermt "10 seconds ago" | wc -l)
...
monitor_eternalblue_alerts() {
    log_message "=== ETERNALBLUE SOAR EXTRACTOR v3.0 STARTED ==="
    ...
    log_message "🚀 Starting EternalBlue monitoring..."
        ...
        if echo "$line" | jq -e '.event_type == "alert"' >/dev/null 2>&1; then
            ...
            if [ -n "${ATTACK_PHASES[$alert_sid]}" ]; then
                ...
                if [ -n "$src_ip" ] && [ -n "$dest_ip" ] && [ -n "$timestamp" ]; then
                    ...
                    log_message "   🎯 Target: $src_ip → $dest_ip"
                    log_message "   📝 Signature: $alert_signature"
                    log_message "   🆔 SID: $alert_sid"
                    
                    # Store event for correlation
                    store_attack_event "$src_ip" "$dest_ip" "$timestamp" "$phase" "$alert_signature" "$line"
                    
                    # Control extraction concurrency and prevent duplicates
                    local bg_jobs=$(jobs -r | wc -l)
                    if [ "$bg_jobs" -gt 3 ]; then
                        log_message "⏸️ Extraction queue full ($bg_jobs jobs), waiting..."
                        wait
                    fi
                    
                    # Use enhanced extraction with deduplication
                    extract_eternalblue_traffic_enhanced "$src_ip" "$dest_ip" "$timestamp" "$alert_signature" "$phase" "$line" &
                    
                    # Brief delay to prevent system overload
                    sleep 2
                fi
            else
                # Check for legacy EternalBlue signatures (backward compatibility)
                if echo "$alert_signature" | grep -qiE "eternalblue|trans2|wannacry|petya|doublepulsar|exploit.*smb"; then
                    local src_ip=$(echo "$line" | jq -r '.src_ip // empty')
                    local dest_ip=$(echo "$line" | jq -r '.dest_ip // empty')
                    local timestamp=$(echo "$line" | jq -r '.timestamp // empty')
                    local phase="PHASE_1_LEGACY_UNKNOWN"
                    
                    if [ -n "$src_ip" ] && [ -n "$dest_ip" ] && [ -n "$timestamp" ]; then
                        log_message "🚨 LEGACY ETERNALBLUE PATTERN DETECTED"
                        log_message "   🎯 Target: $src_ip → $dest_ip"
                        log_message "   📝 Signature: $alert_signature"
                        
                        # Store and extract with legacy handling
                        store_attack_event "$src_ip" "$dest_ip" "$timestamp" "$phase" "$alert_signature" "$line"
...
cleanup() {
    log_message "🛑 Shutting down EternalBlue SOAR Extractor..."
    ...
    mkdir -p "$CORRELATION_DIR/archived"
...
# Create archive directory for correlation data
mkdir -p "$CORRELATION_DIR/archived"
